...
class InputMonitor:
    def __init__(self, screen_w, screen_h):
        ...
        for dev in self.devices:
            ...
            # 监听键盘 (EV_KEY) 和 相对位移鼠标 (EV_REL)
            if ecodes.EV_KEY in cap or ecodes.EV_REL in cap:
    ...
    def _loop(self):
        # 使用 select 多路复用监听所有设备文件描述符
        device_map = {dev.fd: dev for dev in self.monitored_devs}
        
        while self.running:
            # 阻塞直到有事件发生
            r, _, _ = select.select(device_map, [], [])
                ...
                try:
                    for event in dev.read():
                        ...
                        # 处理按键
                            ...
                            # event.value: 1=按下, 0=松开, 2=长按重复
                            if event.value in [0, 1]: 
                                key_name = ecodes.KEY.get(event.code, "UNKNOWN")
                                # 格式化键名以匹配 YAML (例如 'KEY_A' -> 'a', 'KEY_ENTER' -> 'enter')
                                if isinstance(key_name, str):
                                    clean_name = key_name.replace("KEY_", "").lower()
                                    if clean_name == "return": clean_name = "enter"
                                    
                                    if event.value == 1:
                                        self.active_keys.add(clean_name)
                                    else:
                                        self.active_keys.discard(clean_name)
                except OSError:
                    # 设备可能已拔出
                    pass
...
class Keyboard:
    def __init__(self, ctx, key_yaml):
        self.ctx = ctx
        self.key_layers = {}
        # 移除 pynput 监听器，改由外部传入 active_keys
...
class MouseMapping:
    def __init__(self, window, window_height, alpha_opt, pad_area):
        self.window = window # GLFW 窗口对象
        self.window_h = window_height
        self.y_offset = alpha_opt[0] if alpha_opt else 0
        self.min_alpha = alpha_opt[1] if alpha_opt else 0.4
    ...
    def update(self, current_win_pos, global_mouse_pos):
        ...
        # 使用 GLFW 调整透明度 (替代 Win32 API)
        if is_hover and not self.is_translucent:
            glfw.set_window_opacity(self.window, self.min_alpha)
            self.is_translucent = True
        elif not is_hover and self.is_translucent:
            glfw.set_window_opacity(self.window, 1.0)
            self.is_translucent = False
...
class Cat:
    ...
    def get_skeleton(self):
        ...
        # 从 InputMonitor 获取全局鼠标位置
        global_mouse = self.input_monitor.get_mouse_pos()
        map_x, map_y = self.input_mapper.update(curr_pos, global_mouse)
...
if __name__ == '__main__':
    ...
    if not os.access('/dev/input/event0', os.R_OK):
        print("Warning: 没有权限读取 /dev/input/ 设备。")
        print("请使用 'sudo python main.py' 运行，或将用户加入 'input' 组。")
